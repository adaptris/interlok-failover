...
public class JGroupsListener extends AbstractListener implements Receiver {
  ...
  public JGroupsListener(Properties bootstrapProperties) {
    ...
    setjGroupsConfigFile(getPropertyValue(bootstrapProperties, FAILOVER_JGROUPS_CONFIG_KEY));
    setjGroupsClusterName(getPropertyValue(bootstrapProperties, FAILOVER_JGROUPS_CLUSTER_KEY));
  }

  @Override
  public void start() throws Exception {
    JGroupsChannel jGroupsChannel = JGroupsChannel.getInstance();
    jGroupsChannel.setConfigFileLocation(getjGroupsConfigFile());
  ...
  private void sendPingEvent(byte[] data) {
    ...
    if (PacketHelper.isPrimaryPing(pingRecord)) {
      sendPrimaryPingEvent(pingRecord);
    } else {
      sendSecondaryPingEvent(pingRecord);
    }
  }
